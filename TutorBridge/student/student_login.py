...
def student_login_window(parent_root):
    win = Toplevel()
    win.title("Student Login - TutorBridge")
    ...
    # Title Layout
    Label(card, text="STUDENT LOGIN", font=("Helvetica", 20, "bold"), fg=accent_color, bg=card_color).pack(pady=(0, 5))
    Label(card, text="Log in to search verified tutors", font=("Helvetica", 10), fg=text_muted, bg=card_color).pack(pady=(0, 20))
    ...
    def login():
        ...
        try:
            ...
            if student:
                messagebox.showinfo("Success", f"Welcome back, {student['name']}!", parent=win)
                win.destroy()
                
                # Launch Student Dashboard
                from TutorBridge.student.search_tutor import student_dashboard
                student_dashboard(parent_root, student["student_id"])
            else:
